...
<div class="filter-bar"><form class="filter-form" method="GET"><input name="search" value="{{ request('search') }}" placeholder="Search name, phone or username"><select name="router_id"><option value="">All routers</option>@foreach($routers as $router)<option value="{{ $router->id }}" @selected((string) request('router_id') === (string) $router->id)>{{ $router->name }}</option>@endforeach</select><select name="connection_port"><option value="">All live ports</option>@foreach($connectionPorts as $port)<option value="{{ $port }}" @selected(request('connection_port') === $port)>{{ $port }}</option>@endforeach</select><select name="status"><option value="">All statuses</option><option value="active" @selected(request('status') === 'active')>Active</option><option value="suspended" @selected(request('status') === 'suspended')>Suspended</option></select><button class="button secondary">Filter</button></form><form class="bulk-sync-form" method="POST" action="{{ route('customers.sync-all') }}" data-confirm="Sync all {{ $customers->total() }} customers matching the current filters to RADIUS?">@csrf<input type="hidden" name="search" value="{{ request('search') }}"><input type="hidden" name="router_id" value="{{ request('router_id') }}"><input type="hidden" name="connection_port" value="{{ request('connection_port') }}"><input type="hidden" name="status" value="{{ request('status') }}"><button class="button primary" @disabled($customers->total() === 0)>Sync all ({{ $customers->total() }})</button></form></div>
<article class="panel"><div class="table-wrap"><table><thead><tr><th>Customer</th><th>PPPoE username</th><th>Package</th><th>Router</th><th>Live port</th><th>Expires</th><th>Status</th><th></th></tr></thead><tbody>
@forelse($customers as $customer)<tr>
    <td><strong>{{ $customer->name }}</strong><small>{{ $customer->phone ?: 'No phone' }}</small></td><td><code>{{ $customer->username }}</code></td><td>{{ $customer->package?->name ?? '—' }}</td><td>{{ $customer->router->name }}</td><td>@if($customer->connection_port)<strong>{{ $customer->connection_port }}</strong><small>{{ $customer->connection_service ?: 'Live RADIUS session' }}</small>@else<span>—</span><small>No live accounting</small>@endif</td>
