...
<form class="form-card form-grid" method="POST" action="{{ route('customers.import.store') }}" enctype="multipart/form-data">@csrf
    ...
    <label>When username already exists<select name="duplicate_action"><option value="skip" @selected(old('duplicate_action', 'skip') === 'skip')>Skip existing customer</option><option value="update" @selected(old('duplicate_action') === 'update')>Update existing customer</option></select><small class="form-help">RADIUS usernames are unique across every router.</small></label>
    <label class="check"><input type="checkbox" name="sync_to_mikrotik" value="1" @checked(old('sync_to_mikrotik'))> Sync each imported row to RADIUS immediately</label>
    <p class="form-help full">Recommended for migration: import with immediate sync off, verify customer/package/status counts, then sync one test customer before using Sync all. These files and the RADIUS authentication table contain PPPoE passwords; protect database access and delete upload files after use.</p>
