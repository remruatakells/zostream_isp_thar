...
<form class="form-card form-grid" method="POST" action="{{ route('customers.import.store') }}" enctype="multipart/form-data">@csrf
    ...
    <p class="form-help full">Every imported row is written to RADIUS automatically; no manual Sync is required. Test one account before removing legacy MikroTik PPP Secrets. These files and the RADIUS authentication table contain PPPoE passwords, so protect database access and delete upload files after use.</p>
