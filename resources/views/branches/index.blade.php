...
<form class="form-card form-grid" method="POST" action="{{ route('branches.store') }}" style="margin-bottom:20px">@csrf
    ...
    <label>OTT deduction (₹)<input type="number" name="ott_deduction" value="{{ old('ott_deduction') }}" min="0" step="0.01" placeholder="Default: ₹{{ config('services.zostream_subscription.ott_deduction', 50) }}"><small class="form-help">Leave blank to use the global OTT deduction.</small></label>
...
<article class="panel"><div class="table-wrap"><table><thead><tr><th>Branch</th><th>Operator share</th><th>OTT deduction</th><th>Packages</th><th>Status</th><th>Customers</th><th></th></tr></thead><tbody>
@forelse($branches as $branch)<tr>
    ...
    <td>₹<input form="branch-update-{{ $branch->id }}" type="number" name="ott_deduction" value="{{ $branch->ott_deduction }}" min="0" step="0.01" placeholder="{{ config('services.zostream_subscription.ott_deduction', 50) }}" aria-label="OTT deduction"></td>
...
</tr>@empty<tr><td colspan="7" class="empty">No branch created yet.</td></tr>@endforelse
