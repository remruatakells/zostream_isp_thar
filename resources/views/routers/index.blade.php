...
@section('content')
<div class="page-actions"><div><h2>MikroTik routers</h2><p>REST API connections used for provisioning PPPoE customers.</p></div><a class="button primary" href="{{ route('routers.create') }}">+ Add router</a></div>
...
@forelse($routers as $router)<tr>
    ...
    <td><span class="badge {{ $router->is_active ? '' : 'off' }}">{{ $router->is_active ? 'Active' : 'Disabled' }}</span></td>
